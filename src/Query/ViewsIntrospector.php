...
class ViewsIntrospector
{
    ...
    public function get(): Collection
    {
        ...
        return collect($views)->filter(fn ($view) => $this->matchesFilters($view));
    }

    /**
     * Get the first matching view.
     *
     * @return string|null First matching view name or null
     */
    ...
    private function findViewsInPath(string $path, ?string $namespace = null, string $prefix = ''): array
    {
        ...
        foreach ($files as $file) {
            ...
            if (is_dir($fullPath)) {
                $nestedPrefix = $prefix ? "{$prefix}.{$file}" : $file;
     ...
     *
     * @param  string $view      View name to check
     * @param  array  $filterSet Set of filters to match against
    ...
    private function matchesFilterSet(string $view, array $filterSet): bool
    {
        ...
        if (isset($filterSet['doesnt_extend']) && $this->extends($view, $filterSet['doesnt_extend'])) {
            return false;
        }
    ...
    private function uses(string $view, string $pattern): bool
    {
        ...
        foreach ($matches[1] as $includedView) {
            if ($this->matchesPattern($includedView, $pattern)) {
                return true;
            }
        }
     ...
     * @return string|null View content or null if not found
     */
