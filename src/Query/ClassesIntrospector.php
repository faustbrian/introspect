...
class ClassesIntrospector
{
     ...
     *
     * @return Collection<int, string> Collection of class names
     *
     * @example
     * ```php
     * $classes = Introspect::classes()
    ...
    public function get(): Collection
    {
        ...
        return collect($classes)->filter(fn ($class) => $this->matchesFilters($class));
    }

    /**
     * Get the first matching class.
     *
     * @return string|null First class name or null if none found
     *
     * @example
     * ```php
    ...
    private function matchesFilters(string $class): bool
    {
        ...
        if (! $this->matchesPrimaryFilters($class)) {
            return false;
        }

        // Check OR filters (if any)
        if (! empty($this->orFilters)) {
            foreach ($this->orFilters as $orQuery) {
                if ($orQuery->matchesPrimaryFilters($class)) {
                    return true;
                }
            }
    ...
    {
        if (isset($this->filters['name']) && ! $this->matchesPattern($class, $this->filters['name'])) {
            return false;
        }

        if (isset($this->filters['starts_with']) && ! str_starts_with($class, $this->filters['starts_with'])) {
            return false;
        }

        if (isset($this->filters['ends_with']) && ! str_ends_with($class, $this->filters['ends_with'])) {
            return false;
        }

        if (isset($this->filters['contains']) && ! str_contains($class, $this->filters['contains'])) {
            return false;
        }

        if (isset($this->filters['extends']) && ! extendsClass($class, $this->filters['extends'])) {
            return false;
        }

        if (isset($this->filters['implements']) && ! implementsInterface($class, $this->filters['implements'])) {
            return false;
        }

        if (isset($this->filters['uses']) && ! usesTrait($class, $this->filters['uses'])) {
            return false;
        }
