...
final class JobsIntrospector
{
    ...
    {
        $this->filters[] = function (string $job) use ($trait): bool {
            /** @var class-string $job */
            return Reflection::usesTrait($job, $trait);
        };
    ...
    {
        $this->filters[] = function (string $job): bool {
            /** @var class-string $job */
            return Reflection::implementsInterface($job, ShouldBeUnique::class);
        };
    ...
    public function whereEncrypted(): static
    {
        $this->filters[] = function (string $job): bool {
            /** @var class-string $job */
            return Reflection::implementsInterface($job, ShouldBeEncrypted::class);
        };
     ...
     *
     * @return bool True if at least one job matches
     */
    ...
    {
        return $this->get()->map(function (string $job): array {
            /** @var class-string $job */
            return [
                'class' => $job,
     ...
     *
     * Determines if a class is a queue job by checking if it implements ShouldQueue
     * or has "Job" in its class/namespace name.
     *
     * @param  string $class Fully-qualified class name
     * @return bool   True if class is a queue job
     */
     ...
     *
     * Evaluates both main filters (AND logic) and OR filters to determine if a job matches.
     *
     * @param  string $job Fully-qualified job class name
     * @return bool   True if job matches filter criteria
     */
     ...
     *
     * @param  string $job Fully-qualified job class name
     * @return bool   True if job passes all main filters
     */
     ...
     *
     * Extracts the static queue property or returns null for dynamic queues.
     *
     * @param  string      $job Fully-qualified job class name
     * @return null|string Queue name or null if not determinable
     */
     ...
     *
     * Extracts the static connection property or returns null for dynamic connections.
     *
     * @param  string      $job Fully-qualified job class name
     * @return null|string Connection name or null if not determinable
     */
     ...
     *
     * Extracts the static tries property or returns null for dynamic tries.
     *
     * @param  string   $job Fully-qualified job class name
     * @return null|int Number of tries or null if not determinable
     */
     ...
     *
     * Extracts the static backoff property or returns null for dynamic backoff.
     *
     * @param  string              $job Fully-qualified job class name
     * @return null|array<int>|int Backoff setting (seconds or array of seconds) or null
     */
     ...
     *
     * Returns middleware configuration for the job. Note that middleware is typically
     * dynamic and requires instantiation, so this returns empty array in most cases.
     *
     * @param  string             $job Fully-qualified job class name
     * @return array<int, string> Array of middleware class names
     */
