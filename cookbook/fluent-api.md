...
class ServiceDiscovery
{
    public function discoverServices(): array
    {
        ...
        $interfaces = Introspect::interfaces()
            ->whereNameStartsWith('App\Contracts')
            ->whereNameEndsWith('Service')
            ->get();

        foreach ($interfaces as $interface) {
            // Find implementing class
            $implementationName = str_replace('Contracts', 'Services', $interface);

            if (class_exists($implementationName)) {
                $class = Introspect::class($implementationName)
...
class ModelAuditor
{
    public function auditModels(array $models): array
    {
        $report = [];

        foreach ($models as $model) {
            $info = Introspect::class($model)
                ->toArray();

            $report[] = [
                'model' => $info['short_name'],
                'uses_soft_deletes' => Introspect::class($model)
                    ->whereUsesTrait(SoftDeletes::class)
                    ->passes(),
                'has_timestamps' => Introspect::class($model)
                    ->whereHasProperty('timestamps')
                    ->passes(),
                'fillable_count' => count(
                    Introspect::class($model)
                        ->whereHasProperty('fillable')
                        ->passes() ? $model::make()->getFillable() : []
                ),
            ];
        }
...
{
    public function validate(object $plugin): bool
    {
        return Introspect::instance($plugin)
            ->whereImplements(PluginInterface::class)
            ->whereHasPublicMethod('boot')
            ->whereHasPublicMethod('register')
            ->passes();
    }

    public function getPluginInfo(object $plugin): array
    {
        $introspector = Introspect::instance($plugin);

        return [
            'name' => $introspector->getBasename(),
            'namespace' => $introspector->getNamespace(),
            'implements' => $introspector->toArray()['interfaces'],
            'capabilities' => $introspector->getPublicMethods(),
        ];
    }
}
```
